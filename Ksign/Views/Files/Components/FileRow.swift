...
struct FileRow: View {
    ...
    private func fileConfirmationDialogButtons() -> some View {
        ...
        if file.isKsignFile {
            Button {
                UIAlertController.showAlertWithOk(title: "?", message: .localized("NicheLoader certificate file (.ksign) is now unsupported, please refer to use .p12 and .mobileprovision instead."))
            } label: {
